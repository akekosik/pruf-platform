...
Защита от «сфотографировал экран и спросил модель» не может быть обещанием в
презентации. Она должна быть метрикой, которая падает в CI, если интерфейс
случайно начнёт отдавать лишнее.

SSR (screenshot solve rate) — верхняя оценка того, насколько шаг НЕ решается из кадра:
...
ssr_static = 1 означает «из кадра не решается вообще»: чем больше, тем лучше.
Порог для решающих шагов — SSR_GATE.

Инварианты, которые проверяются механически
    I1 no_enumerable_answer  у решающего задания нет списка вариантов;
    I2 no_answer_in_payload  в payload нет эталонов, свидетелей, кода мутанта;
...
# Стоимость одного цикла релея в секундах.
RELAY_COST = {"capture": 6.0, "upload": 5.0, "model": 12.0, "readback": 9.0}
# Перенабрать строку кода с фотографии — около 4,5 с на строку.
...
# Поведенческие сигналы. Это НЕ прокторинг: ни камеры, ни экрана, ни
# биометрии. Смотрим только на процесс работы внутри инструмента.
# Сигналы не меняют балл — они только помечают сессию к пересмотру человеком.
RISK_SIGNALS = {
...
CLAIM = "Информация(фотография экрана) ⊆ Информация(клиентский payload)"


def _cv(values: List[float]) -> float:
    """Коэффициент вариации — разброс, независимый от масштаба."""
...
def relay_cost_model(task_type: str, code_lines: int = 26) -> Dict[str, Any]:
    ...
    A1 — только фотография кадра: цикл дешёвый, но ответа не даёт.
    A2 — второй компьютер с запуском кода. Здесь мы честны: бюджет времени сам по
    себе атаку не закрывает. Закрывает то, что атакующему придётся восстановить
    решение строка за строкой, собрать мутанта и прогнать его — то есть выполнить
    ту же инженерную работу, которую мы и измеряем.
    """
...
def threat_model(code_lines: int = 26) -> List[Dict[str, Any]]:
    """Три сценария обхода и что с каждым из них делает протокол."""
    return [
        {
            "id": "A1",
            ...
            "why": (
                ...
                "вход и два наблюдаемых значения, нужно запустить код — фотография этого не даёт."
            ),
        ...
        },
        {
            ...
            "status": "удорожается",
            "why": (
                "Придётся восстановить решение строка за строкой, собрать мутанта и прогнать "
                "его, то есть сделать ту же работу, которую мы и проверяем."
            ),
...
def audit(
    ...
    invariants = [
        {
            ...
            "ok": not enumerable,
            "detail": (
                "варианты найдены у: %s" % ", ".join(map(str, enumerable))
                if enumerable
                else "все решающие шаги требуют артефакта"
            ),
        },
        {
            "id": "I2",
            ...
            "ok": not leaks,
            "detail": (
                "утечки: %s" % ", ".join(leaks[:8]) if leaks else "запрещённых ключей нет"
            ),
        },
        {
            "id": "I3",
            ...
            "detail": (
                "без запуска: %s" % ", ".join(map(str, no_execution))
                if no_execution
                else "решающих шагов: %d" % len(decisive)
            ),
        },
        {
            "id": "I4",
            ...
            "ok": not weak_frames,
            "detail": (
                "слабые шаги: %s" % ", ".join(weak_frames)
                if weak_frames
                else "ssr_static ≥ %.2f на всех решающих шагах" % SSR_GATE
            ),
        },
        {
            "id": "I5",
    ...
    return {
        ...
        "ssr": ssr_rows,
        "ssr_gate": SSR_GATE,
        "threat_model": threat_model(code_lines),
